...
# ╔═╡ b94dd394-12f9-490c-aad0-56635f1f65ea
md""" 

## State your budget (time and money)
...
# ╔═╡ 8b697c10-f6fb-4f30-a83d-11c73c6b6e7a
md"""

## State your theory

We can't claim that we're a real science without quantitative theory building (cf. Alday 2015, Guest and Martin 2021).

"""

# ╔═╡ 5da5bb89-b3ee-458f-b290-41ae9187548e
html"""
<blockquote class="twitter-tweet" data-conversation="none" data-lang="en" data-dnt="true"><p lang="en" dir="ltr">Modelling is letting the ideas that run in your head also run on other computers.</p>&mdash; Olivia Guest · Ολίβια Γκεστ (@o_guest) <a href="https://twitter.com/o_guest/status/1220786717399113728?ref_src=twsrc%5Etfw">January 24, 2020</a></blockquote> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script> 
"""
...
md"**Relative**  By-subject standard deviation for `frequency`-`context` interaction: $(@bind s_subj_interaction  NumberField(0:0.1:5; default=0.1))"
...
md"**Relative**  By-item standard deviation for `frequency`-`context` interaction: $(@bind s_item_interaction  NumberField(0:0.1:5; default=0.1))"
...
begin
	...
	function simulate_fit_different_form(
		...
		if use_threads
			...
			Threads.resize_nthreads!(data_threads)
		end
		...
		samp = replicate(n; use_threads=use_threads, hide_progress=hide_progress) do
			...
			local data = data_threads[tidx]
			lock(rnglock)
			mod = simulate!(rng, mod)
			unlock(rnglock)
			data[!, fitform.lhs.sym] = response(mod)
...
md"""The model fit to the **true** data-generating formula:"""

# ╔═╡ a6e3bf43-3047-4dcf-a278-02451db3a817
m0

# ╔═╡ 8652fb19-6232-4c02-81c5-0c7318a58325
md"""Model with simplified formula fit to the same data:"""

# ╔═╡ 540d24c7-2132-4a37-ae0d-33b7a8ee009e
m1 = fit(MixedModel, fitform, design; contrasts=contrasts)

# ╔═╡ bf20a161-0d37-4e49-a7b9-e79c33cbbbe8
md"""
!!! note
    The estimates themselves don't change much. This is already a hint at something bigger: **replication** (which always means more participants) is critical to check results. Notably, in the case where the misspecification has the biggest difference in terms of *stargazing*, we fail to recover the true parameter value
"""
...
kb07contrasts = Dict(:spkr => EffectsCoding(), :prec => EffectsCoding(), :load => EffectsCoding())
